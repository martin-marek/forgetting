...
def _make_random_cfg(template_cfg, hidden_size=None, num_hidden_layers=None):
    ...
    head_dim = _head_dim(template_cfg)
    if hidden_size < head_dim or hidden_size % head_dim != 0:
        raise ValueError(f"hidden_size must be a positive multiple of head_dim={head_dim}, got {hidden_size}")
    ...
    cfg["num_attention_heads"] = hidden_size // head_dim
    template_kv_heads = template_cfg.get("num_key_value_heads", template_cfg["num_attention_heads"])
    cfg["num_key_value_heads"] = math.gcd(cfg["num_attention_heads"], template_kv_heads)
    cfg["intermediate_size"] = template_cfg["intermediate_size"] * hidden_size // template_cfg["hidden_size"]
    if "head_dim" in cfg: cfg["head_dim"] = head_dim
...
def load(model_id="alpindale/Llama-3.2-1B", hf_ckpt_dir="/dev/shm/ptx/weights", tp_size=1, dp_shard=False, init="pretrained", hidden_size=None, num_hidden_layers=None, key=None):
